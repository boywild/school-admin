  ...
  <page-header-wrapper>
    <a-card :bordered="false">
              ...
              <a-col :md="8" :sm="24">
                <a-form-item label="班型">
                  <a-select v-model="queryParam.vipClassFlag" placeholder="请选择班型">
                    <a-select-option :value="item.code" v-for="(item, index) in dict.ClassTypeEnum" :key="index">{{
                      item.desc
                    }}</a-select-option>
                  </a-select>
                </a-form-item>
      ...
      <div class="table-operator">
        ...
        <a-button type="primary" ghost icon="download" v-action:T080 @click="downLoad">导出数据</a-button>
        <a-button type="primary" ghost icon="upload" v-action:T080 @click="importExcel">导入数据</a-button>
      </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return studentList({ applyType: 'S009', ...requestParameters }).then(res => {
          console.log(res)
          return res
        })
      },
  ...
  methods: {
    ...
    async downLoad() {
      await downLoadExcel('S009')
    },
    importExcel() {
      this.$router.push({ name: 'sysImport', query: { applyType: 'S009' } })
    }
  }
}
</script>
